...
namespace TryAR.MarkerTracking
{
    ...
    public class ArUcoMarkerTracking : MonoBehaviour
    {
        ...
        [Header("Debug/Editor Input")]
        [Tooltip("拖拽WebCamTextureManagerPrefab到此处")]
        ...
        void Start()
        {
        // 通过 webcamManager 获取 webcamTexture
        ...
        {
            Debug.LogWarning("webcamManager is not assigned! Inspector拖拽WebCamTextureManagerPrefab到此字段。");
        }
        // 不在Start()创建resultTexture，避免尺寸不匹配。只在Update()里创建。
        }

        void Update()
        {
        // 每帧都获取最新 webcamTexture
        if (webcamManager != null)
            webcamTexture = webcamManager.webcamTexture;

        Debug.Log($"Update called | WebCamTexture size: {webcamTexture?.width}x{webcamTexture?.height} | isPlaying: {webcamTexture?.isPlaying}");

        // 在Editor下，webcamTexture准备好后初始化ArUco追踪
        #if UNITY_EDITOR
        if (!_isReady && webcamTexture != null && webcamTexture.isPlaying && webcamTexture.width > 0 && webcamTexture.height > 0)
        {
            // 这里的参数可以根据实际情况调整
            float fx = 800; // 近似焦距
            float fy = 800;
            float cx = webcamTexture.width / 2f;
            float cy = webcamTexture.height / 2f;
            Initialize(webcamTexture.width, webcamTexture.height, cx, cy, fx, fy);
            Debug.Log("ArUco tracking initialized for Editor webcam.");
        }
        #endif

        if (webcamTexture != null && webcamTexture.isPlaying)
        {
            int processedWidth = webcamTexture.width / _divideNumber;
            int processedHeight = webcamTexture.height / _divideNumber;
            // 只有在resultTexture为null或尺寸不匹配时才重新创建
            if (resultTexture == null || resultTexture.width != processedWidth || resultTexture.height != processedHeight)
            {
                resultTexture = new Texture2D(processedWidth, processedHeight, TextureFormat.RGBA32, false);
                Debug.Log($"ResultTexture created: {resultTexture.width}x{resultTexture.height}");
            }
            // 确保尺寸匹配后再调用DetectMarker
            if (resultTexture.width == processedWidth && resultTexture.height == processedHeight)
            {
                Debug.Log("Calling DetectMarker...");
                DetectMarker(webcamTexture, resultTexture);
            }
            else
            {
                Debug.LogWarning($"Skip DetectMarker: resultTexture size {resultTexture.width}x{resultTexture.height} != processed size {processedWidth}x{processedHeight}");
            }
        }
        }
        ...
        {
        Debug.Log($"DetectMarker called. _isReady: {_isReady}, webCamTexture: {(webCamTexture != null ? "not null" : "null")}, webCamTexture size: {webCamTexture?.width}x{webCamTexture?.height}");
        if (!_isReady)
        {
            Debug.LogWarning($"DetectMarker skipped: _isReady is false. _processingRgbMat: {(_processingRgbMat == null ? "null" : "ok")}, _originalWebcamMat: {(_originalWebcamMat == null ? "null" : "ok")}, _halfSizeMat: {(_halfSizeMat == null ? "null" : "ok")}, arucoDetector: {(arucoDetector == null ? "null" : "ok")}, markerDictionary: {(markerDictionary == null ? "null" : "ok")}");
            return;
        }
        if (webCamTexture == null)
        {
            Debug.Log("webCamTexture is null, skipping detection.");
            return;
        }
        ...
        if (resultTexture != null)
        {
            ...
            Debug.Log("Detected marker count: " + _detectedMarkerCorners.Count);
        }
